...
The IDE supports developing applications with a *Klive project*. With the **File | Open folder** command, you can open the contents of a particular folder and display them. The IDE considers folders with a well-formed `klive.project` file within them a *Klive project*.
...
- `build.ksx` is a script file that executes within the IDE. It contains functions representing build commands.

By changing the functions' body, you can customize the build process (and create new build-related commands).


When you open a folder that is not a Klive project (does not have a valid `klive.project` file), you cannot see the home icon:

<ClickableImage src="/images/getting-started/non-klive-project.png" width="248" />

## The Build Root

When you develop your code, you may work with multiple files — Klive needs to know the main file to compile into machine code. The *build root* designates this main file. When you create a new project, Klive automatically assigns a file as the build root. In the Explorer, the "combine" icon to the right of the file name indicates this:

<ClickableImage src="/images/getting-started/build-root-sign.png" width="248" />

You can change the build root at any time by right-clicking on a file and selecting the **Promote to Build Root** command.

<ClickableImage src="/public/images/getting-started/promote-build-root.png" width="320" />
